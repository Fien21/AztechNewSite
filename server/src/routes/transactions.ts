...
router.post('/', authenticate, authorize('ADMIN', 'CASHIER'), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { items, discount } = req.body;

        // Check if POS is open
        const settings = await db.get<any>('SELECT * FROM Settings LIMIT 1');
        ...
        let totalAmount = 0;
        const itemsData: Array<{ productId: number; quantity: number; price: number; subtotal: number; name: string; isBundle: boolean }> = [];
        ...
        for (const item of items) {
            ...
            // Validate stock
            if (product.isBundle) {
                const bundleItems = await db.all<any>('SELECT bi.*, p.name, p.stockQty FROM BundleItem bi JOIN Product p ON bi.productId = p.id WHERE bi.bundleId = ?', [product.id]);
        ...
        const resultId = await db.transaction(async (tx) => {
            const { id: transactionId } = await tx.run(
                'INSERT INTO Transaction_Table (cashierId, totalAmount, discount) VALUES (?, ?, ?)',
                [req.user!.id, totalAmount, discountAmount]
            );
            ...
            for (const item of itemsData) {
                ...
                // Deduct stock
                if (item.isBundle) {
                    const bundleItemsArr = await tx.all<any>('SELECT * FROM BundleItem WHERE bundleId = ?', [item.productId]);
        ...
        const finalTxn = await db.get<any>(`
        ...
        const finalItems = await db.all<any>(`
    ...
    try {
        ...
        for (const txn of transactions) {
            ...
            const items = await db.all<any>(`
                ...
                WHERE ti.transactionId = ?
            `, [txn.id]);
            txn.items = items.map((i: any) => ({ ...i, product: { name: i.productName } }));
        }
...
router.get('/:id', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const items = await db.all<any>(`
            ...
            WHERE ti.transactionId = ?
        `, [id]);
        transaction.items = items.map((i: any) => ({ ...i, product: { name: i.productName } }));
...
// GET dashboard stats
router.get('/stats/dashboard', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const productStats = await db.get<any>('SELECT COUNT(*) as totalProducts, (SELECT COUNT(*) FROM Product WHERE stockQty <= 10) as lowStockProducts FROM Product');
        const { count: totalTransactions } = await db.get<any>('SELECT COUNT(*) as count FROM Transaction_Table');
        const { count: totalCategories } = await db.get<any>('SELECT COUNT(*) as count FROM Category');
        ...
        const todaySales = await db.get<any>(`
        ...
        // Recent transactions
        const recentTransactions = await db.all<any>(`
...
// GET chart data
router.get('/stats/charts', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { interval = 'week' } = req.query;
        const now = new Date();
        let startDate = new Date();

        if (interval === 'day') startDate.setHours(now.getHours() - 24);
        else if (interval === 'week') startDate.setDate(now.getDate() - 7);
        else if (interval === 'month') startDate.setDate(now.getDate() - 30);
        else if (interval === 'year') startDate.setFullYear(now.getFullYear() - 1);
        ...
        const dataMap: Record<string, { label: string, sales: number, units: number }> = {};
